...
func SendCommand(hostname string, fileName string) error {
	...
	longCommand := "sh -c 'curl https://raw.githubusercontent.com/byuoitav/raspi-deployment-microservice/master/docker-compose.yml --output /tmp/docker-compose.yml && curl " + os.Getenv("RASPI_DEPLOYMENT_MICROSERVICE_ADDRESS") + "/" + fileName + " --output /home/pi/.environment-variables && . ~/.bashrc && docker-compose -f /tmp/docker-compose.yml pull && docker stop -f $(docker ps -a -q) || true && docker rm -f $(docker ps -a -q) || true && docker-compose -f /tmp/docker-compose.yml up -d --remove-orphans'"
