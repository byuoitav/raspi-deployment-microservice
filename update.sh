...
docker kill pjlink-microservice
docker rm pjlink-microservice
docker run --net="host" -e LOCAL_ENVIRONMENT="true" -d -e PJLINK_PORT=$PJLINK_PORT -e PJLINK_PASS=$PJLINK_PASS --restart=always --name pjlink-microservice -p 8005:8005 byuoitav/rpi-pjlink-microservice:latest
